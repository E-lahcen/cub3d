...
/*   Updated: 2022/08/28 21:19:57 by lelhlami         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    set_mini_map(t_data *data)
{
	...
	limit_top_y = data->player.y - 10;
	limit_right_x = data->player.x + 10;
	limit_bottom_y = data->player.y + 10;
	// print_map(&data->map);
	while (cnt_i < 21)
	{
		cnt_j = 0;
...
void	mini_setup_grid(t_data *data)
{
	...
	data->map.mini_grid = (t_block **)malloc(sizeof(t_block *) * 21);
	i = -1;
	while (++i < 21)
		data->map.mini_grid[i] = (t_block *)malloc(sizeof(t_block) * 21);
	i = -1;
	while (++i < 21)
	{
		j = -1;
